...
try{
  ...
  await context.route('http://127.0.0.1:19198/**',async route=>{
    ...
    if(/^\/tutorial-audio\/(modes_broadcast|modes_interaction|broadcast|input|edit|clear|reinput|send|switch)\.wav$/.test(path))return route.fulfill({contentType:'audio/wav',body:await readFile(new URL('.'+path,import.meta.url))})
    if(path.endsWith('.mjs'))return route.fulfill({contentType:'text/javascript',body:await readFile(new URL('.'+path,import.meta.url),'utf8')})
    return route.fulfill({contentType:'text/html',body:`<!doctype html><html><head></head><body style="margin:0;background:#111923;color:white;font-family:system-ui"><aside style="width:220px;height:100vh;border-right:1px solid #445"><h2>DeepSeek Harness</h2><div data-slot="sidebar.sessions">教学会话</div><div id="dsh-duplex-voice-controls" style="position:fixed;bottom:30px;left:20px;width:170px;height:35px">duet <button data-voice="speaker">喇叭</button></div></aside><main style="position:absolute;left:270px;top:40px"><h2>对话</h2></main><div data-composer-input style="position:fixed;bottom:40px;left:270px;width:750px;min-height:55px;border:1px solid #678;padding:12px"></div><script type="module">
      import {mountTutorial} from '/deepseek_harness_voice_tutorial.mjs';
      ...
      window.tutorial=mountTutorial({controls:document.querySelector('#dsh-duplex-voice-controls'),adapter});
    </script></body></html>`})
  })
  const page=await context.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message))
  await page.goto('http://127.0.0.1:19198/')
  await page.evaluate(()=>{
    const controls=document.querySelector('#dsh-duplex-voice-controls')
  ...
  assert.equal(await page.locator('.live-tutorial-highlight').getAttribute('data-target'),'[data-voice="speaker"]')
  assert.equal(await page.locator('.live-tutorial-pointer').isVisible(),true)
  await page.screenshot({path:'/tmp/dsh-tutorial-speaker-intro.png'})
  await page.getByRole('heading',{name:'双向箭头 · 交互模式'}).waitFor()
  assert.equal(await page.locator('.live-tutorial-highlight').getAttribute('data-target'),'[data-voice="mic"]')
  assert.equal(await page.locator('.live-tutorial-pointer').isVisible(),true)
  ...
  assert.equal(await page.locator('.live-tutorial button').count(),1)
  assert.equal(await page.locator('.live-tutorial-mode svg').count(),2)
  ...
  await click('完成');await page.reload();assert.equal(await page.locator('.live-tutorial').count(),0)
  await page.goto('http://127.0.0.1:19198/?live_tutorial=1');await click('不再提醒');await page.reload()
  assert.equal(await page.locator('.live-tutorial').count(),0)
  ...
  const manifest=JSON.parse(await readFile(new URL('./tutorial-audio/manifest.json',import.meta.url),'utf8'))
  ...
  await page.getByRole('checkbox').nth(0).check();await page.getByRole('checkbox').nth(1).check();await click('开始教学')
  await page.getByRole('heading',{name:'想改哪里，直接告诉我'}).waitFor().catch(async e=>{console.log(await page.locator('.live-tutorial').innerText());console.log(await page.evaluate(()=>calls));throw e})
  await click('退出教学')
