...
import { VoiceState, PLUGIN_VERSION, compatibilityNotice } from './deepseek_harness_voice_state.mjs'

test('mode upgrade is actionable and never silently switches mode', async () => {
  const notices = [], connections = []
  const state = new VoiceState({ notice: (...v) => notices.push(v), connect: mode => {
...
test('recommendation is nonblocking and appears once', async () => {
  const notices = []
  const state = new VoiceState({ notice: (...v) => notices.push(v), connect: () => ({ close() {}, sendState() {} }) })
