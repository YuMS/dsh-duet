...
import { compatibleVersion, externalWithState } from '../../src/host/proxy.mjs'

const flush = () => new Promise(resolve => setImmediate(resolve))
function fixture() {
  let now = 0
  const connections = [], notices = []
  const state = new DuetState({ now: () => now, notice: text => notices.push(text), connect(mode, receive) {
...
test('PCM acknowledgements follow playback time, not received bytes; old audio stays cleared', () => {
  const audio = new DuetAudio(), sent = []
