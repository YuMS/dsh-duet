...
import { MICROPHONE_KEY, selectedMicrophone, microphoneConstraints } from '../../src/client/microphone.mjs'
...
test('capture uses selected device; missing device never retries a different microphone', async t => {
  ...
  await assert.rejects(new DuetAudio().startMic(()=>{}), {name:'OverconstrainedError'})
