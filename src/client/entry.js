  ...
  factory: () => {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        let active = true
        let dispose
        import('/duet/assets/client/browser.mjs?v=0.1.1').then(module => {
      ...
      }, "duet-client: bridge current composer")
      ...
      }, "duet-client: bridge server composer RPC")
      ...
      ctx.effect(() => {
        ...
        const heartbeat = async () => {
          ...
          try {
            ...
            if (!controller.signal.aborted) console.warn("duet heartbeat failed:", error)
          } finally {
        ...
        const publishSelection = async () => {
          ...
          try {
            ...
            if (!controller.signal.aborted) console.warn("duet selection publish failed:", error)
          } finally {
        ...
        const poll = async () => {
          ...
          try {
            ...
            if (!controller.signal.aborted) console.warn("duet focus sync failed:", error)
          } finally {
            pollRunning = false
          }
        }
      ...
      }, "duet-client: synchronize host and browser focus")
    }
