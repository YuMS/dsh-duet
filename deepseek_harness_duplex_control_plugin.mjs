/** DeepSeek Harness plugin that exposes the duplex control console on the official WebUI host. */

import { readFile } from 'node:fs/promises'
import { randomUUID } from 'node:crypto'
import { registerVoiceProxy, voiceConfig } from './deepseek_harness_voice_proxy.mjs'
import { VoiceSettings, settingsWriteAllowed } from './deepseek_harness_voice_settings.mjs'
import { ServiceNotices } from './deepseek_harness_voice_notices.mjs'
import { ServiceInfo } from './deepseek_harness_voice_service_info.mjs'
...
import { RemoteFeedback } from './deepseek_harness_voice_feedback_files.mjs'
import { PLUGIN_VERSION } from './deepseek_harness_voice_state.mjs'
import { buildSessionCatalog } from './deepseek_harness_voice_catalog.mjs'
...
const DUPLEX_RUNTIME_STALE_SECONDS = 3
const HTML_URL = new URL('./deepseek_harness_duplex_control_plugin.html', import.meta.url)
...
export function apply(ctx, options = {}) {
  ...
  let lastDuplexRuntimeSeenAt = 0
  let lastNativeInteractionsSeenAt = -Infinity
  const hostAbort = new AbortController()

  const voice = voiceConfig()
  const settings = new VoiceSettings(voice, { path: options.settingsPath, debug: options.debug === true || process.env.DUPLEX_CONTROL_DEBUG === '1' })
  Object.assign(voice, settings.effective())
  const audit = new VoiceTrace({ kind: 'host', plugin_version: PLUGIN_VERSION, pid: process.pid },
  ...
  let voiceProxy
  ctx.effect(() => { voiceProxy = registerVoiceProxy(ctx, voice, feedback); return voiceProxy })

  ctx.effect(() => () => { hostAbort.abort() })

  const duplexRuntimeActive = () => (
    nowSeconds() - lastDuplexRuntimeSeenAt <= DUPLEX_RUNTIME_STALE_SECONDS
  )
  ...
  ctx.on('approval/request', function (request, next) {
    if (!duplexRuntimeActive() || nativeInteractionsActive()) return next()
  ...
  ctx.effect(() => () => {
    for (const interaction of pendingInteractions.values()) {
      finishInteraction(interaction, () => {
        interaction.reject(codedError('plugin_stopped', 'Duplex control plugin stopped'))
      })
    }
  })
  ...
  const handleApi = async (req, res, pathname) => {
    ...
    if (req.method === 'GET' && pathname === `${ROUTE}/api/voice/config`) {
      ...
      sendJson(res, 200, { ...settings.public(), plugin_version: PLUGIN_VERSION, feedback_enabled: feedback.enabled, connection: voiceProxy?.snapshot?.() || { online: 0, tts_only: 0 } })
      return
    }
    if (req.method === 'PUT' && pathname === `${ROUTE}/api/voice/config`) {
      ...
      try {
        ...
        voiceProxy?.closeConnections?.('voice_settings_changed')
    ...
    if (req.method === 'GET' && pathname === `${ROUTE}/api/health`) {
      sendJson(res, 200, {
        ...
        duplex_runtime_connected: duplexRuntimeActive(),
        duplex_runtime_last_seen_at: lastDuplexRuntimeSeenAt || null,
        duplex_runtime_stale_after_seconds: DUPLEX_RUNTIME_STALE_SECONDS,
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && tutorialAudio) {
        const rejection = ctx.connection.requestRejection(req)
        if (rejection) { sendError(res, rejection, '请先打开并登录 DSH 主页面'); return }
        const body = await readFile(new URL(`./tutorial-audio/${tutorialAudio[1]}.wav`, import.meta.url))
        res.writeHead(200, { 'content-type': 'audio/wav', 'cache-control': 'private, max-age=3600', 'content-length': body.length })
        res.end(body)
        return
      }
      const asset = pathname.match(/^\/duplex-control\/(deepseek_harness_voice_(?:state|audio|microphone|shortcuts|catalog|browser|rpc|composer|interactions|tutorial|tutorial_adapter|tutorial_request|feedback|consent)\.mjs)$/)
      if (req.method === 'GET' && asset) {
        const body = await readFile(new URL(`./${asset[1]}`, import.meta.url))
        res.writeHead(200, { 'content-type': 'text/javascript; charset=utf-8', 'cache-control': 'no-store' })
        res.end(body)
        return
      }
      if (String(req.headers?.['x-duplex-control-client'] || '').trim()) {
        lastDuplexRuntimeSeenAt = nowSeconds()
      }
