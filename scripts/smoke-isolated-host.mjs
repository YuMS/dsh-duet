...
try {
  ...
  await page.route('**/deepseek_harness_voice_browser.mjs*', async route => {
    const response = await route.fetch()
    const body = (await response.text())
      .replace('export function mountVoice(ctx) {', 'export function mountVoice(ctx) { globalThis.__duetAccess = composerAccess(ctx); globalThis.__duetCtx = ctx;')
      .replace('const state = new VoiceState(', 'const state = globalThis.__duetState = new VoiceState(')
  ...
  assert.equal(await page.locator('#dsh-duplex-voice-controls').count(), 1)
  ...
  assert.equal(await page.locator('#dsh-duplex-control-entry').innerText(), 'duet.')
  ...
  const result = await page.evaluate(async id => {
    ...
    const { observeComposer } = await import('/duplex-control/deepseek_harness_voice_composer.mjs')
    const observations = []
  ...
  assert.equal(await page.locator('#dsh-duplex-voice-controls').count(), 0)
  ...
  await page.evaluate(async()=>{
    const {saveShortcut}=await import('/duplex-control/deepseek_harness_voice_shortcuts.mjs')
  ...
  await page.locator('[data-voice="mic"]').click()
  const notice = page.locator('#dsh-duplex-voice-controls [role="status"]')
