...
import {consumeTutorialRequest} from './deepseek_harness_voice_tutorial_request.mjs?v=0.1.0'
...
export class TutorialRun {
  constructor(adapter) {
    ...
    this.first = null; this.second = null; this.expected = ''; this.voiceOwned = false
  ...
  async broadcast(consent) { return this.operation(async () => {
    ...
    this.assertFocus(this.first); this.voiceOwned = true
    ...
    await this.a.off(); this.voiceOwned = false; this.step = 'input'
  }) }
  ...
  async stop() {
    ...
    if (this.voiceOwned) { this.voiceOwned = false; await this.a.off() }
  }
}
...
export function mountTutorial({ controls, adapter }) {
  ...
  style.textContent = `
    .live-tutorial-pointer{position:fixed;z-index:100011;pointer-events:none;color:#80c2ff;filter:drop-shadow(0 2px 5px #0008)}
    .live-tutorial-mode{display:flex;align-items:center;gap:12px;margin:14px 0}.live-tutorial-mode svg{width:24px;height:24px;color:#80c2ff;flex-shrink:0}
    .live-tutorial{position:fixed;z-index:100010;width:min(380px,calc(100vw - 28px));max-height:calc(100vh - 28px);overflow:auto;box-sizing:border-box;padding:22px;color:#edf3ff;background:#17212f;border:1px solid #52647d;border-radius:18px;box-shadow:0 14px 50px #0007;font:14px/1.65 system-ui,sans-serif}
    .live-tutorial h3{font-size:19px;margin:4px 0 12px}.live-tutorial p{white-space:pre-wrap;margin:10px 0}.live-tutorial small{color:#b1c5dc}.live-tutorial footer{display:flex;gap:8px;flex-wrap:wrap;margin-top:18px}.live-tutorial button{font:inherit;padding:7px 13px;border:1px solid #66809c;border-radius:9px;background:transparent;color:inherit;cursor:pointer}.live-tutorial button.primary{background:#80c2ff;color:#112035;border-color:transparent;font-weight:650}.live-tutorial button:disabled{opacity:.45;cursor:wait}.live-tutorial label{display:flex;align-items:flex-start;gap:10px;margin:12px 0;cursor:pointer}.live-tutorial input{margin-top:6px}.live-tutorial-highlight{position:fixed;z-index:100009;pointer-events:none;border:2px solid #80c2ff;border-radius:12px;box-shadow:0 0 0 5px #80c2ff25;transition:all .15s}.live-tutorial-help{border:0;background:transparent;color:inherit;cursor:pointer;font:600 12px system-ui;padding:5px}
  `
  let target = controls, targetSelector = '#dsh-duplex-voice-controls'
  const position = () => {
    if (!panel) return
  ...
  const show = (title, text, actions=[], selector, finished=false) => {
    ...
    targetSelector = selector || '#dsh-duplex-voice-controls'
    target = document.querySelector(targetSelector) || controls
    highlight = element('div',null,document.body); highlight.className='live-tutorial-highlight'; highlight.setAttribute('aria-hidden','true')
    highlight.dataset.target=targetSelector
    if(selector==='[data-voice="speaker"]'||selector==='[data-voice="mic"]'){
      pointer=document.createElementNS('http://www.w3.org/2000/svg','svg');pointer.classList.add('live-tutorial-pointer')
    ...
    panel=element('section',null,document.body); panel.className='live-tutorial'; panel.setAttribute('role','region'); panel.setAttribute('aria-label','duet 使用教学')
  ...
  const done = () => {
    ...
    for(const [kind,text] of [['mic','交互模式'],['speaker','播报模式']]){
      const row=element('div');row.className='live-tutorial-mode'
